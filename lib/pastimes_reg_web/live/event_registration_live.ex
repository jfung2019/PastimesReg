defmodule PastimesRegWeb.EventRegistrationLive do
  ...
  def mount(%{"id" => event_id, "name" => category_name}, _session, socket) do
    event = Events.get_event!(event_id)

    changeset =
      Participants.event_registration_form_step_init_changeset(
        %PastimesReg.Participants.Participant{}
      )
  ...
  def handle_event(
    ...
    socket =
      socket
      |> assign(
        changeset: Participants.event_registration_form_step_1_changeset(attrs),
        current_step: 3
      )
